...
void mainWidget::initComboMonth()
{
    ...
    QStringList month;
    QDateTime current_time = QDateTime::currentDateTime()  ; //当前时间

    for(int i=1;i<=10;i++){
      month.append(current_time.addMonths(-i).toString("yyyy-MM"));//当前月份前10个月
		// 此处为固定时间和日期
		// 请使用QDate/QDateTime将其修正，
		// 用户运行前一个月开始连续10个月的"年-月"
		// (如2018-02、2018-01、2017-12...，假设当前日期为2018年3月12日)
        //month<<QString(current_time);//时间写成动态的qdatatime日期时间
    }
    ui->comboMonth->clear();
